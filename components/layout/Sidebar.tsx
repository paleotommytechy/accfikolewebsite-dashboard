...
import { NAV_LINKS, ADMIN_LINKS, BLOG_LINKS } from '../../constants';

const Sidebar: React.FC = () => {
  const { isSidebarOpen, isAdmin, isBlogger, isProfileComplete } = useAppContext();
  ...
  return (
    ...
    `}>
      ...
      <nav className="flex flex-col p-4 space-y-2 flex-grow overflow-y-auto">
        {NAV_LINKS.map((link) => {
          const isDisabled = !isProfileComplete && !['/dashboard', '/profile', '/blog'].includes(link.href);
          return (
